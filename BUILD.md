...
How the Gradle build for `azure-app-svc` is put together, what each task
...
> **Note** — `logging.level.com.rubensgomes.azure.appsvc` is set to `trace` in the
...
```bash
kill $(pgrep -f com.rubensgomes.azure.appsvc.App)
```
...
```bash
./gradlew bootJar
java -jar app/build/libs/azure-app-svc-<version>.jar
```
...
| Archive                                  | Contents                                                 |
|------------------------------------------|----------------------------------------------------------|
| `azure-app-svc-<version>.jar`         | executable Spring Boot jar (layered, for Docker caching) |
| `azure-app-svc-<version>-plain.jar`   | library jar, classes only                                |
| `azure-app-svc-<version>-sources.jar` | sources                                                  |
| `azure-app-svc-<version>-javadoc.jar` | Javadoc                                                  |
...
| `sonar.projectKey`       | `rubensgomes-org_azure-app-svc` |
| `sonar.projectName`      | `azure-app-svc`                 |
...
there is no `azure-app-svc-spring-boot.jar` for the Dockerfile and no `.env`.
