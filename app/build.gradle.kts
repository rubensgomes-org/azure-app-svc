 ...
 * ---------------------------------------------------------------------
 *
 * azure-app-svc Gradle build script (Kotlin DSL)
 *
...
dependencies {
    ...
    // springdoc: generates the OpenAPI 3 document from the Spring MVC mappings
    // and serves the Swagger UI console.
...
//
// The "title", "description" and "version" tokens work the same way and feed
// the "openapi.*" block that OpenApiConfig binds, so the OpenAPI document
// describes itself with the same values the published POM carries. "version"
// in particular is rewritten by the release plugin on every release; reading
// it from a token means the served spec follows without a second edit.
//
...
tasks.processResources {
    val replacedTokens =
        mapOf(
            "artifactId" to gradleProperty("artifactId"),
            "description" to gradleProperty("description"),
            "title" to gradleProperty("title"),
            "version" to gradleProperty("version"),
        )
    replacedTokens.forEach { (name, value) -> inputs.property(name, value) }
    filesMatching("application*.yml") {
        filter<org.apache.tools.ant.filters.ReplaceTokens>(
            "tokens" to replacedTokens,
        )
    }
}
...
tasks.javadoc {
    ...
    // "@apiNote", "@implSpec" and "@implNote" are documented by the JDK but are
    // not registered in the standard doclet outside the JDK's own build, so a
    // user-code occurrence fails as an "unknown tag". Registering them here is
    // what lets the class headers carry the "@implNote" the java-spring
    // guidelines require.
...
// excludes = listOf("com.rubensgomes.azure.appsvc.config.*")
...
publishing {
    publications {
        create<MavenPublication>("maven") {
            ...
            // Gradle project directory ("app") instead of "azure-app-svc".
...
tasks.spotbugsMain {
    reports {
        ...
        // pinned rather than left to the convention because the CI logs
        // name these paths.
        //
        ...
        create("xml") {
            required.set(true)
            outputLocation.set(
                layout.buildDirectory.file("reports/spotbugs/main.xml"),
            )
        }
    }
}
...
release {
    // Expanded by the plugin through a Groovy SimpleTemplateEngine whose
    // binding holds "version" and "name". The default is an unprefixed
    // "$version", which would produce a bare 0.0.1 tag.
    //
    // The prefix is for consistency with the sibling azure-iac and
    // azure-workflows repositories. Nothing depends on it: the release
    ...
    //
    // The dollar sign is escaped so Kotlin passes the token through instead of
    // interpolating it. tagTemplate belongs to the release extension itself,
    // NOT to the nested git block -- GitConfig has no such property.
...
val applicationJar =
    tasks.register<Copy>("applicationJar") {
        ...
        description =
            "Copies the Spring Boot multi-layer jar to $artifactId-spring-boot.jar"
        from(tasks.bootJar)
...
val generateDotEnv =
    tasks.register("generateDotEnv") {
        description =
            "Writes APP_VERSION to the root .env read by docker compose."
